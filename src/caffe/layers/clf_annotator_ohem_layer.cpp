...
namespace caffe {
  ...
  void ClfAnnotatorOHEMLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
    ClfAnnotatorOHEMParameter box_anno_param = this->layer_param_.clf_annotator_ohem_param();
  ...
  template <typename Dtype>
  void ClfAnnotatorOHEMLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
    num_ = bottom[0]->num();
  ...
  template <typename Dtype>
  void ClfAnnotatorOHEMLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
    NOT_IMPLEMENTED;
  }

  template <typename Dtype>
  void ClfAnnotatorOHEMLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
#ifdef CPU_ONLY
  STUB_GPU(ClfAnnotatorOHEMLayer);
#endif

  INSTANTIATE_CLASS(ClfAnnotatorOHEMLayer);
  REGISTER_LAYER_CLASS(ClfAnnotatorOHEM);
