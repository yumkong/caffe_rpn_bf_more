...
void RpnCenterLossNewLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
...
template <typename Dtype>
void RpnCenterLossNewLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  ...
  CHECK_EQ(bottom[0]->height(), bottom[1]->height()); // output blob dim[1] == label dim[1] (original hei)
  CHECK_EQ(bottom[0]->width(), bottom[1]->width()); // output blob dim[2] == label dim[2] (original wid)
  CHECK_EQ(bottom[1]->channels(), 1); //label's chl
  CHECK_EQ(bottom[2]->channels(), 1); // label_weight's chl
  //###liu### change for rpn loss layer
  //M_ = bottom[0]->num();
  M_ = bottom[0]->num() * bottom[0]->height() * bottom[0]->width(); //
  spatial_ = bottom[0]->height() * bottom[0]->width();
  // The top shape will be the bottom shape with the flattened axes dropped,
  // and replaced by a single axis with dimension num_output (N_).
  // liu@0803: NOTE this is loss layer's reshape: output a scalar with 0 axes
...
template <typename Dtype>
void RpnCenterLossNewLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
  ...
  for (int i = 0; i < M_; i++) {
    ...
    {
        //liu@0803: since the address is not consecutive, write separately
        // given: num x hei x wid  ==> [num, 0, hei, wid]
        *(distance_data + i / spatial_ * K_ * spatial_ + i % spatial_) = *(bottom_data + i / spatial_ * K_ * spatial_ + i % spatial_) - *(center + label_value * K_);
        // given: num x hei x wid  ==> [num, 1, hei, wid]
        *(distance_data + i / spatial_ * K_ * spatial_ + spatial_ + i % spatial_) = *(bottom_data + i / spatial_ * K_ * spatial_ + spatial_ + i % spatial_) - *(center + label_value * K_);
...
template <typename Dtype>
void RpnCenterLossNewLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
    ...
    const vector<Blob<Dtype>*>& bottom) 
{
  ...
  if (this->param_propagate_down_[0]) {
    ...
    Dtype* variation_sum_data = variation_sum_.mutable_cpu_data(); // 2 x 2
    // liu 0716: initialized as 0
        ...
        if (label_weight_value > 0 && label_value == n) 
	{
          ...
          *(variation_sum_data + n * K_) -= *(distance_data + m / spatial_ * K_ * spatial_ + m % spatial_);
          *(variation_sum_data + n * K_ + 1) -= *(distance_data + m / spatial_ * K_ * spatial_ + spatial_ + m % spatial_);
        }
      }
      // a * x + y
      count_sum += count;
...
#ifdef CPU_ONLY
STUB_GPU(RpnCenterLossNewLayer);
#endif

INSTANTIATE_CLASS(RpnCenterLossNewLayer);
REGISTER_LAYER_CLASS(RpnCenterLossNew);
